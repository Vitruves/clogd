    ...
    def _perform_training_analysis(self, X, y):
        ...
        if self.clusters is not None:
             for cluster_id in sorted(np.unique(self.clusters)):
                 ...
                 if np.sum(cluster_mask) > 0:
                     ...
                     self.error_analysis[cluster_id] = cluster_stats
        ...
        try:
             analysis_data = {
                 ...
                 'error_analysis_per_cluster': self.error_analysis,
    ...
    def predict(self, X, smiles=None):
        ...
        if self.important_indices is not None:
            ...
            if self.feature_cols and X.shape[1] != len(self.feature_cols):
                 print_status(f"Warning: Prediction input feature count ({X.shape[1]}) differs from training feature count ({len(self.feature_cols)}). Attempting alignment.")
                 X_aligned = np.full((X.shape[0], len(self.feature_cols)), self.replace_nan if self.replace_nan is not None else 0.0)
                 # This assumes X's columns correspond to the start of self.feature_cols - NEEDS FEATURE NAMES FOR ROBUSTNESS
                 n_cols_match = min(X.shape[1], len(self.feature_cols))
                 X_aligned[:, :n_cols_match] = X[:, :n_cols_match]
                 print_status(f"Aligned prediction input to {X_aligned.shape[1]} features before selection.")
                 ...
                 if max(self.important_indices) < X.shape[1]:
                     X_pred_base = X[:, self.important_indices]
                      ...
                      raise ValueError("Important indices out of bounds for prediction input.")
        ...
        if args.external_test and args.external_test != input_source:
             ...
             try:
                  ...
                  if len(existing_ext_feats) == len(model.feature_cols):
                       ...
                       print_status(f"Generating/Aligning features for external test set...")
                       ...
                       if gen_names == model.feature_cols:
                            ...
                            print_status("Alignment complete.")
